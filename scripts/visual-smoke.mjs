...
const chartsModulePath = pathToFileURL(
  resolve(__dirname, "../src/assets/js/lib/charts.js"),
).href;
const { drawBarChart, drawRadarChart } = await import(chartsModulePath);

const hashOps = (ops) =>
  createHash("sha256").update(JSON.stringify(ops)).digest("hex");
...
class MockContext {
  ...
  setLineDash(args) {
    this._record("setLineDash", { args: Array.from(args) });
  }
...
const BAR_EXPECTED_HASH =
  "87c562186949ab0a652caa04324722322c4d5dbb55fba7cdb2df009e395fe001";
const RADAR_EXPECTED_HASH =
  "423801e12384d5d08ef998a871cb98f0968263f0cec3e7463ba925eab0b21f00";
...
if (barHash !== BAR_EXPECTED_HASH) {
  failures.push(
    `Bar chart render hash mismatch. Expected ${BAR_EXPECTED_HASH}, got ${barHash}.`,
  );
}

if (radarHash !== RADAR_EXPECTED_HASH) {
  failures.push(
    `Radar chart render hash mismatch. Expected ${RADAR_EXPECTED_HASH}, got ${radarHash}.`,
  );
}
...
console.log(
  "Visual smoke passed: render operations match expected fingerprints.",
);

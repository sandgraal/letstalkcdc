module.exports = {
  ...
  quizConfig: {
    ...
    description: "Test your understanding of partition key selection and ordering guarantees.",
    questions: [
      {
      ...
      },
      {
        ...
        correct: "2",
        explanation: "Partition skew occurs when data is unevenly distributed across partitions, often due to poor key selection (e.g., using a tenant ID when one tenant dominates traffic). This creates hotspots—overloading some partitions/brokers while others sit idle—leading to reduced throughput, increased latency, and consumer lag."
      },
      {
        ...
        correct: "2",
        explanation: "Late-arriving events occur when network delays, retries, multi-partition reads, or replays cause an older event to arrive after a newer one. Without proper handling (timestamps, watermarks, or versioning), consumers might apply changes out of order, corrupting state (e.g., overwriting new data with old)."
      },
      {
        question: "How can you mitigate partition skew caused by a single hot key?",
        options: [
      ...
      },
      {
        ...
        correct: "2",
        explanation: "An audit loop (or reconciliation job) compares the source database with the downstream sink to detect discrepancies—missing events, late arrivals, or out-of-order updates. This provides a safety net, catching issues that might slip through the streaming pipeline due to partitioning challenges or failures."
      }
    ]
  }
};

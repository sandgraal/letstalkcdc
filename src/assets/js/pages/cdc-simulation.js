...
const _exists = (selector) => doc.querySelector(selector) !== null;
...
function generateRandomEvent() {
  ...
  if (op === "insert") {
    ...
    const record =
      state.records[Math.floor(Math.random() * state.records.length)];
    return {
      op: "UPDATE",
      table: "products",
      before: { ...record },
      after: {
        ...record,
        stock: record.stock + Math.floor(Math.random() * 20) - 10,
      },
      timestamp: Date.now(),
    };

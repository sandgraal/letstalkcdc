...
onReady(() => {
  ...
  stepperBtn.addEventListener("click", () => {
    ...
    if (currentStep >= steps.length) {
      ...
      diagramVisual.innerHTML =
        '<div class="icon">🎬</div><p>Ready to start</p>';
  ...
  idempotentBtn?.addEventListener("click", () => {
    doc
      .querySelectorAll(".step-diagram .step")
      .forEach((el) => el.classList.remove("active", "completed"));
    diagramVisual.innerHTML =
      '<div class="icon">🧮</div><p>Idempotent upsert: replay is a no-op</p>';
    ...
    const tick = () => {
      if (index < sequence.length) {
        ...
        diagramVisual.innerHTML =
          '<div class="icon">🧊</div><p>Replay hits same <code>event_id</code> → MERGE matches → 0 rows changed</p>';
      }
    };

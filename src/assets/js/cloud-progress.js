...
async function initCloudProgress() {
  ...
  if (!endpoint || !project || !databaseId) {
    console.info('Appwrite not configured; cloud sync unavailable');
    return false;
  }
...
async function mergeProgressOnLogin(userId) {
  ...
  const localVisits = getVisitedModules();
  ...
  window.dispatchEvent(new CustomEvent('cdc:progress-synced', {
    detail: { 
      userId,

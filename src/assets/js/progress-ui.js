...
function showBadgesNotification() {
  ...
  // Manual dismiss
  notification.querySelector('[data-dismiss-badges]')?.addEventListener('click', () => {
    notification.classList.add('fade-out');
    setTimeout(() => notification.remove(), 300);
  });
}
...
function addCompletionButton() {
  ...
  // Handle click
  buttonContainer.querySelector('[data-mark-complete]')?.addEventListener('click', () => {
    window.dispatchEvent(new CustomEvent('cdc:mark-complete', {

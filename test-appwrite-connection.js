...
const ASSISTANT_COLLECTION =
  process.env.COL_ASSISTANT_ID || "assistant_feedback";
...
if (
  ...
  console.error(
    "Set APPWRITE_ENDPOINT, APPWRITE_PROJECT, APPWRITE_DB_ID, and APPWRITE_API_KEY.",
  );
  process.exit(1);
}
...
async function listDatabases() {
  ...
  if (!response.ok) {
    throw new Error(
      `Unable to list databases (${response.status} ${response.statusText})`,
    );
  }
...
async function listCollections(databaseId) {
  const response = await fetch(
    `${APPWRITE_ENDPOINT}/databases/${databaseId}/collections`,
    { headers },
  );
  if (!response.ok) {
    throw new Error(
      `Unable to list collections (${response.status} ${response.statusText})`,
    );
  }
  return response.json();
}
...
async function main() {
  try {
    ...
    if (!collections.collections?.length) {
      console.error(
        "❌ No collections found. Import appwrite.collections.json.",
      );
      process.exit(1);
    }
    ...
    console.log(
      "3. Submit assistant feedback and verify the document appears.",
    );
  } catch (error) {

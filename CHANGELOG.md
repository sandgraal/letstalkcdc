## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

# Changelog

All notable changes to Let's Talk CDC will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

## [Unreleased]

### Added
- Periodic content review system for CDC tool versions
- Tool versions tracking in `src/_data/toolVersions.cjs`
- Content review agent (`site-content-review`) for quarterly reviews
- Quarterly review checklist generation

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-06
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-05
- Initial changelog

## 2025-11-02
- Initial changelog

## 2025-11-02
- Initial changelog

## 2025-11-01
- Initial changelog

## 2025-11-01
- Initial changelog

## 2025-11-01
- Initial changelog

### Added
- Content review infrastructure for tracking CDC ecosystem updates

## 2025-10-26

### Changed
- Exposed a `CDCProgress.refresh` helper to force Appwrite syncs and keep cached dashboard data current
- Added `CDCProgress.getDashboardDocs` for normalized dashboard access from client scripts

## 2025-10-25 and earlier

### Added
- Initial site content and infrastructure
- Multiple learning modules and quickstart guides
- AI agent system for automated maintenance
- Analytics and monitoring capabilities

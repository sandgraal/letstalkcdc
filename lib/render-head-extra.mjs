...
export function renderHeadExtra(
  ...
  return (
    str
      ...
      // Render-blocking-resources fix: convert any `<link rel="stylesheet"
      // href="X">` emitted by a page's head_extra block to the preload +
      // onload pattern with a <noscript> fallback. The base layout's own
      // three stylesheet links are converted inline; centralizing here
      // means every page that injects a page-specific stylesheet via
      // head_extra (intro.css, snapshotting.css, ...) gets the fix
      // without editing each page's front-matter.
      .replace(
        /<link\s+rel=(?:"stylesheet"|'stylesheet')\s+href=("[^"]+"|'[^']+')\s*\/?>/g,
        (_, href) =>

 ...
 * @returns {string}
 */
// Match any stylesheet <link> inside `head_extra`, regardless of whether
// attributes appear as rel→href or href→rel. The two href capture groups
// handle double-quoted and single-quoted values respectively so we can
// normalize the rewritten output to safe double-quoted HTML attributes.
const HEAD_EXTRA_STYLESHEET_LINK_RE =
  /<link\b(?=[^>]*\brel=(?:"stylesheet"|'stylesheet'))(?=[^>]*\bhref=(?:"([^"]*)"|'([^']*)'))[^>]*\/?>/g;
...
const renderDeferredStylesheetLink = (href) => {
  const safeHref = escapeAttributeValue(href);
  return (
...
export function renderHeadExtra(
  ...
  return str
    ...
    .replace(HEAD_EXTRA_STYLESHEET_LINK_RE, (_, hrefDouble, hrefSingle) =>
      renderDeferredStylesheetLink(hrefDouble ?? hrefSingle ?? ""),
    );
}

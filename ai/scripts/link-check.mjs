...
class LinkChecker {
  ...
  async findHtmlFiles(dir, files = []) {
    try {
      ...
      console.warn(
        `[link-check] Could not read directory ${dir}: ${err.message}`,
      );
    }
  ...
  async check() {
    ...
    if (!existsSync(SITE_DIR)) {
      console.error(
        "[link-check] Error: _site directory not found. Run `npm run build` first.",
      );
    ...
    console.log(
      `[link-check] Found ${htmlFiles.length} HTML file(s) to check.`,
    );
    ...
    console.log(
      `[link-check] Checked ${totalLinks} link(s) across ${htmlFiles.length} file(s).`,
    );
    ...
    if (this.brokenLinks.length > 0) {
      console.error(
        `[link-check] ❌ Found ${this.brokenLinks.length} broken link(s):`,
      );

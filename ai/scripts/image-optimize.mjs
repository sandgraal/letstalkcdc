...
async function optimizeImages() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "[image] sharp not installed. Run: npm install --save-dev sharp",
      );
    ...
    console.log(
      `[image] Found ${images.length} image(s) to check for optimization.`,
    );
    ...
    for (const imagePath of images) {
      ...
      try {
        ...
        if (!hasWebP && ext !== ".webp") {
          console.log(
            `[image] Would optimize: ${imagePath.replace(PROJECT_ROOT, ".")} (${sizeMB} MB)`,
          );
          ...
          if (CONFIG.enabled) {
            ...
            await sharp(imagePath)
              ...
              .toFile(webpPath);
            console.log(
              `[image] ✅ Optimized to WebP: ${webpPath.replace(PROJECT_ROOT, ".")}`,
            );
          }
    ...
    console.log(
      `[image] Summary: ${optimized} ${CONFIG.enabled ? "optimized" : "would be optimized"}, ${skipped} already optimized or skipped.`,
    );
    if (!CONFIG.enabled) {
      console.log(
        "[image] Note: Actual optimization is disabled. Set IMAGE_OPTIMIZE_ENABLED=true to enable.",
      );
    }
  } catch (err) {

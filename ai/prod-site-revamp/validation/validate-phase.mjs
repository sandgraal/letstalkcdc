...
function validatePhase1_1() {
  ...
  const subdirBuild = exec(
    "grep -c 'href=\"/letstalkcdc/' _site/index.html",
    true,
  );
  check(
...
function validatePhase1_2() {
  ...
  if (fileExists("src/assets/js/app.js")) {
    const appLines = execSync("wc -l src/assets/js/app.js", {
      encoding: "utf-8",
    });
...
function validatePhase1_3() {
  ...
  const hashedFiles = exec(
    'find _site/assets/js -name "*.*.js" 2>/dev/null | wc -l',
    true,
  );
  check(
  ...
  const bundleSize = exec(
    "du -k _site/assets/js/*.js 2>/dev/null | sort -n | tail -1",
    true,
  );
